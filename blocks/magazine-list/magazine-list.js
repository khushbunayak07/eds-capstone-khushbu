...
/**
 * Dynamic magazine listing block.
 *
 * Fetches the magazine query-index, sorts entries by date (newest first), and
 * renders one card per row — image (optimized <picture>), title, description,
 * and a link to the article. Replaces the static author-placed article cards.
 *
 * Authoring: the block's first cell may optionally override the query-index
 * path (e.g. a link or plain text). Defaults to the section's own
 * `query-index.json` resolved relative to the current page.
 */
...
function toTimestamp(value) {
  ...
  if (typeof value === 'number') {
    // EDS date columns are commonly epoch seconds; scale up if it looks like seconds
...
function buildCard(row, doc) {
  ...
  if (row.image) {
    ...
    imageCell.className = 'magazine-list-card-image';
...
export default async function decorate(block) {
  ...
  try {
    ...
    // leave rows empty on failure; block renders nothing rather than breaking the page
    rows = [];
  }
  ...
  rows.forEach((row) => ul.append(buildCard(row, document)));
  block.append(ul);
}
