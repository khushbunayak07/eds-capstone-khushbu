...
/**
 * Dynamic adventures listing block.
 *
 * Fetches the adventures query-index and renders one card per row — image
 * (optimized <picture>), title, description, and a link to the adventure.
 * Replaces the static author-placed adventure cards.
 *
 * Authoring: the block's first cell may optionally override the query-index
 * path (a link or plain text). Defaults to the adventures section index.
 */
...
function buildCard(row, doc) {
  ...
  // image cell — optimized picture wrapped in the adventure link
  if (row.image) {
    const imageCell = doc.createElement('div');
    imageCell.className = 'adventures-list-card-image';
  ...
  body.className = 'adventures-list-card-body';
